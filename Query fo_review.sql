   ...
   SELECT idx, datex, linex, b.PART_NAME partdx, c.LINE_DESC linedx, groupx, partx, byx, stg, a.ss, a.rej, avgppm, totavgppm,
	a.seam, a.spi, a.dim, a.oth, a.act, a.pflagx, a.procx, a.procdx, a.mcx, a.rev_finding,
	a.ndesc, a.ndim, a.nstre, a.fcpoi, a.fcdesc, a.fcsize, a.fcallow, a.fcstre, a.rev_1, a.rev_2, a.rev_3, a.rev_4, a.rev_5, a.c_stat, a.sort
   FROM (
     SELECT
       c.rev_id idx, c.rev_date datex, c.rev_line linex, c.rev_part partx, c.rev_group groupx, c.rev_by byx, c.rev_stage stg, a.rev_ss ss, a.rev_rej rej,
     ...
     SELECT
       ...
       'AES' pflagx, a.rev_proc_id procx, '' procdx, '' mcx, a.rev_aes_finding rev_finding, '' ndesc, '' ndim, '' nstre,
		 '' fcpoi, '' fcdesc, '' fcsize, '' fcallow, '' fcstre, '' rev_1, '' rev_2, '' rev_3, '' rev_4, '' rev_5, a.rev_aes_stat c_stat, '2' sort
     FROM fo_review_dc_aes a
     LEFT JOIN fo_review_h c ON a.rev_id = c.rev_id
     WHERE a.rev_id = '1'
     UNION ALL
     ...
     SELECT
       ...
       'DMS' pflagx, a.rev_proc_id procx, '' procdx, '' mcx, '' rev_finding, '' ndesc, '' ndim, '' nstre, '' fcpoi, '' fcdesc, '' fcsize, '' fcallow, '' fcstre, '' rev_1,
		 '' rev_2, '' rev_3, '' rev_4, '' rev_5, a.rev_dim_stat c_stat, '5' sort
   ...
   LEFT JOIN toy_part b on a.partx = b.PART_NUM
   LEFT JOIN line c on a.linex = c.LINE_CODE
   ORDER BY fcpoi ASC, sort ASC, procx ASC;
   
   SELECT COUNT(*) FROM fo_review_h
   UNION
   SELECT COUNT(*) FROM fo_review_d
   
   SELECT ROUND((IFNULL(SUM(rev_rej),0) / IFNULL(SUM(rev_ss),0)*1000),0) avgppm
   FROM fo_review_d
   WHERE rev_id='4';
   ...
   FROM fo_review_d
 	...
 	ALTER TABLE fo_review_d
	ADD COLUMN (`rev_act` varchar(255) CHARACTER SET utf8mb3 COLLATE utf8mb3_bin DEFAULT NULL COMMENT 'Action');